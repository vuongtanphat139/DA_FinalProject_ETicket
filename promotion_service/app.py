...
class Event(db.Model):
    event_id = db.Column(db.Integer, primary_key=True, autoincrement=False)
    name = db.Column(db.String(255))
    description = db.Column(db.String(255))
    numberOfTicket = db.Column(db.Integer)
    ...
    def to_dict(self):
        return {
            ...
            'name' : self.name,
            'description': self.description,
            'numberOfTicket': self.numberOfTicket,
            'address': self.address,
        }

# Route để lấy tất cả các đợt khuyến mãi
@app.route('/promotions/getall', methods=['GET'])
def get_incentives():
...
@app.route('/promotions/<int:incentive_id>', methods=['GET'])
...
@app.route('/promotions/<int:incentive_id>', methods=['PUT'])
...
@app.route('/promotions/<int:incentive_id>', methods=['DELETE'])
